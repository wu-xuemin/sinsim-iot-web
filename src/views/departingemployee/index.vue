...
export default {
  ...
  methods: {
    handleEntry(row) {
      this.$confirm(`是否设置员工[${row.name}]为在职?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      })
        .then(() => {
          return entryStaff(row.staffId).then((res) => {
            console.log(res)
            if (res.code == 200) {
              this.$message({
                type: 'success',
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (!this.imageUrl) {
          ...
          if (valid) {
            ...
            updateDepart(this.account, this.temp)
              .then((res) => {
                if (res.code == 200) {
                  this.$message({
                    message: '修改成功',
    ...
    deleteData() {
      this.butLoading = true
      deleteDepart(this.account, this.temp.staffId)
        .then((res) => {
          if (res.code == 200) {
    ...
    //员工列表
    getStaffSearch() {
      return getDepartSearch(this.staffQuery)
        .then((res) => {
