<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-if="role == 1"
                 class="filter-item"
                 style="margin-left: 10px; float: right"
                 type="primary"
                 @click="handleConfigList">配置考勤时间</el-button>
      <el-button v-waves
                 :loading="downloadLoading"
                 class="filter-item"
                 type="primary"
                 style="float: right"
                 @click="handleBu">手动补推</el-button>
    </div>
    ...
    <el-table :data="tableData"
      ...
      <el-table-column prop="staffNumber"
                       label="员工号"
                       align="center">
        <template slot-scope="{ row }">
      ...
      <el-table-column prop="signDeviceName"
                       label="上班打卡设备"
                       align="center">
        <template slot-scope="{ row }">
          <span>{{ row.signDeviceName }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="signOutDeviceName"
                       label="下班打卡设备"
                       align="center">
        <template slot-scope="{ row }">
          <span>{{ row.signOutDeviceName }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="type"
                       ...
                       align="center">
        <template slot-scope="{ row }">
          <span>{{ ReservationType[row.type] }}</span>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="考勤时间段"
               :visible.sync="dialogRuleVisible">
      <el-row>
        <el-button v-if="!configTemp.length"
                   class="filter-item"
                   style="margin-left: 10px"
                   type="primary"
                   @click="handleAddConfig">新增规则</el-button>
      </el-row>
      <el-table :data="configTemp"
                style="width: 100%">
        ...
        <el-table-column label="操作"
          ...
          <template slot-scope="{ row, $index }">
            <i class="el-icon-edit"
               ...
               @click="handleUpdateConfig(row)"></i>
            <!-- <i
      ...
      <el-form :model="pushTime"
        ...
        <el-form-item label="规则名称："
          ...
          <el-input style="width: 220px"
                    v-model="pushTime.name"
                    clearable />
        </el-form-item>
        <el-form-item label="上班时间："
                      prop="startTime"
                      class="is-required">
          <el-time-picker v-model="pushTime.startTime"
                          format="HH:mm:ss"
                          placeholder="上班时间">
          </el-time-picker>
        </el-form-item>
        <el-form-item label="下班时间："
                      prop="endTime"
                      class="is-required">
          <el-time-picker v-model="pushTime.endTime"
                          placeholder="下班时间">
          </el-time-picker>
        ...
        <el-button type="primary"
                   ...
                   :loading="butLoading">配置</el-button>
      </el-form>
    </el-dialog>

    <el-dialog title="手动补推"
               :visible.sync="dialogBuVisible"
               :before-close="handleBuClose">
      <el-form :model="buTime"
               :rules="buRules"
               ref="buTime"
               label-width="100px"
        ...
        <el-form-item label="开始时间："
                      prop="startTime"
                      class="is-required">
          <el-date-picker v-model="buTime.startTime"
                          type="datetime"
                          format="yyyy-MM-dd HH:mm:ss"
                          placeholder="开始时间">
          </el-date-picker>
        ...
        <el-form-item label="结束时间："
                      prop="endTime"
                      class="is-required">
          <el-date-picker v-model="buTime.endTime"
                          type="datetime"
                          format="yyyy-MM-dd HH:mm:ss"
                          placeholder="结束时间">
          </el-date-picker>
        ...
        <el-button type="primary"
                   @click="buPush('buTime')"
                   :loading="butLoading">推送</el-button>
      </el-form>
    </el-dialog>
    ...
    <el-dialog title="提示"
               ...
               width="400px">
      <span>
        确认要删除 [<b>{{ this.pushTime.name }}</b>] 吗？
      </span>
      <span slot="footer"
            class="dialog-footer">
        ...
        <el-button type="primary"
                   @click="deleteData"
                   icon="el-icon-check"
                   :loading="butLoading">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    const validateStartBu = (rule, value, callback) => {
      if (this.buTime.endTime) {
        this.$refs.buTime.validateField('endTime')
      }
      callback()
    }
    const validateEndBu = (rule, value, callback) => {
      // if (!this.pushTime.endTime && this.pushState == 1) {
      //   callback(new Error("请输入结束时间"));
      // } else {

      if (this.buTime.startTime) {
        if (Number(this.buTime.startTime) < Number(value)) {
          callback()
        } else {
    ...
    return {
      ...
      buRules: {
        ...
        startTime: [
          ...
          { validator: validateStartBu, trigger: 'blur' },
        ],
        endTime: [
          { required: true, message: '请输入结束时间', trigger: 'change' },
          { validator: validateEndBu, trigger: 'blur' },
  ...
  methods: {
        ...
        if (valid) {
          ...
          let addQuery = {
            startTime: this.dateFormat(
              'YYYY-mm-dd HH:MM:SS',
              this.buTime.startTime
            ),
            endTime: this.dateFormat(
              'YYYY-mm-dd HH:MM:SS',
              this.buTime.endTime
            ),
            park: this.buTime.park,
          }
          return setAttendanceBu(addQuery)
            .then((res) => {
              if (res.code == 200) {
                this.$message({
                  message: '补推成功',
    ...
    getList() {
      ...
      return getAttendanceSearch(this.listQuery)
        .then((res) => {
    ...
    getExcel() {
      ...
      return getAttendanceExport(excelData)
        .then((res) => {
          if (res.code == 400) {
    ...
    handleBu() {
      this.dialogBuVisible = true
      this.buTime.park = this.listQuery.parkId
    },
    // selectTime() {
    //   // console.log(this.pushTime)
    //   if (this.pushTime && this.pushTime.length > 0) {
